function Map(options) {
  ...
  this.drawMap = function() {
    ...
    if (this.options.geosearch != undefined) {
      var settings = _.extend(this.options.geosearch.settings == undefined ? {} : this.options.geosearch.settings, {zoomLevel: 11});
  ...
  this.drawMarkers = function (data){
    ...
    _.each(data, function(item, index) {
      if (item.Latitude != undefined && item.Longitude != undefined && index <= 25) {
        item.color = activeColor ? activeColor : that.iconColor(item['Services Provided'][0]);
  ...
  this.iconColor = function(service) {
    var color = '';
